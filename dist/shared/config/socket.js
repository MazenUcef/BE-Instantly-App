...
exports.getIO = exports.initSocket = exports.socketEvents = exports.socketRooms = void 0;
const socket_io_1 = require("socket.io");
const jsonwebtoken_1 = __importDefault(require("jsonwebtoken"));
const call_model_1 = __importDefault(require("../../modules/call/models/call.model"));
const session_model_1 = __importDefault(require("../../modules/session/models/session.model"));
let io;
exports.socketRooms = {
    user: (userId) => `user_${userId}`,
    supplierOrders: (categoryId, governmentId) => `category_${categoryId}_government_${governmentId}`,
...
const canAccessSession = async (sessionId, userId) => {
...
const initSocket = (server) => {
    ...
    io.on("connection", (socket) => {
        ...
        if (user?.userId) {
            socket.join(exports.socketRooms.user(user.userId));
        }
        socket.on(exports.socketEvents.JOIN_SESSION_ROOM, ({ sessionId }) => {
            if (!sessionId)
                return;
            const room = exports.socketRooms.chat(sessionId);
            socket.join(room);
            console.log(`Socket ${socket.id} joined session room ${room}`);
            socket.emit(exports.socketEvents.SESSION_ROOM_JOINED, { sessionId });
        });
        socket.on(exports.socketEvents.LEAVE_SESSION_ROOM, ({ sessionId }) => {
            if (!sessionId)
                return;
            const room = exports.socketRooms.chat(sessionId);
            socket.leave(room);
        ...
        socket.on(exports.socketEvents.JOIN_SUPPLIER_ORDER_ROOMS, ({ categoryId, governmentIds, }) => {
            if (user?.role !== "supplier")
                return;
            if (!categoryId || !Array.isArray(governmentIds))
                return;
            for (const governmentId of governmentIds) {
                const room = exports.socketRooms.supplierOrders(categoryId, governmentId);
                socket.join(room);
                console.log(`Socket ${socket.id} joined ${room}`);
            }
            socket.emit(exports.socketEvents.SUPPLIER_ROOMS_JOINED, {
                categoryId,
                governmentIds,
            });
        });
        socket.on(exports.socketEvents.LEAVE_SUPPLIER_ORDER_ROOMS, ({ categoryId, governmentIds, }) => {
            if (!categoryId || !Array.isArray(governmentIds))
                return;
            for (const governmentId of governmentIds) {
                const room = exports.socketRooms.supplierOrders(categoryId, governmentId);
        ...
        socket.on(exports.socketEvents.CALL_START, async ({ sessionId }) => {
            try {
                ...
                const receiverId = session.customerId.toString() === user.userId
                    ? session.supplierId.toString()
                    : session.customerId.toString();
                const existingActiveCall = await call_model_1.default.findOne({
                    sessionId,
                    status: { $in: ACTIVE_CALL_STATUSES },
                });
                if (existingActiveCall) {
                    socket.emit(exports.socketEvents.CALL_BUSY, {
                        sessionId,
                ...
                const call = await call_model_1.default.create({
                    sessionId,
                ...
                io.to(exports.socketRooms.user(receiverId)).emit(exports.socketEvents.CALL_INCOMING, {
                ...
                io.to(exports.socketRooms.user(user.userId)).emit(exports.socketEvents.CALL_RINGING, {
                    callId: call._id.toString(),
                    sessionId,
                    receiverId,
                    type: "audio",
        ...
        socket.on(exports.socketEvents.CALL_ACCEPT, async ({ callId, sessionId }) => {
            try {
                if (!user?.userId || !callId || !sessionId)
                    return;
                const call = await call_model_1.default.findById(callId);
                if (!call) {
                    socket.emit(exports.socketEvents.CALL_ERROR, {
                        sessionId,
                        callId,
                ...
                await call.save();
                io.to(exports.socketRooms.user(call.callerId.toString())).emit(exports.socketEvents.CALL_ACCEPTED, {
                    callId,
                    sessionId,
                    answeredBy: user.userId,
                    timestamp: new Date(),
                });
                io.to(exports.socketRooms.user(call.receiverId.toString())).emit(exports.socketEvents.CALL_ACCEPTED, {
                    callId,
                    sessionId,
                    answeredBy: user.userId,
                    timestamp: new Date(),
        ...
        socket.on(exports.socketEvents.CALL_DECLINE, async ({ callId, sessionId }) => {
            try {
                if (!user?.userId || !callId || !sessionId)
                    return;
                const call = await call_model_1.default.findById(callId);
                ...
                await call.save();
                io.to(exports.socketRooms.user(call.callerId.toString())).emit(exports.socketEvents.CALL_DECLINED, {
                    callId,
                    sessionId,
                    declinedBy: user.userId,
                    timestamp: new Date(),
                });
                io.to(exports.socketRooms.user(call.receiverId.toString())).emit(exports.socketEvents.CALL_DECLINED, {
                    callId,
                    sessionId,
                    declinedBy: user.userId,
                    timestamp: new Date(),
        ...
        socket.on(exports.socketEvents.CALL_END, async ({ callId, sessionId }) => {
            try {
                if (!user?.userId || !callId || !sessionId)
                    return;
                const call = await call_model_1.default.findById(callId);
                ...
                await call.save();
                io.to(exports.socketRooms.user(call.callerId.toString())).emit(exports.socketEvents.CALL_ENDED, {
                    callId,
                    sessionId,
                    endedBy: user.userId,
                    timestamp: new Date(),
                });
                io.to(exports.socketRooms.user(call.receiverId.toString())).emit(exports.socketEvents.CALL_ENDED, {
                    callId,
                    sessionId,
                    endedBy: user.userId,
                    timestamp: new Date(),
        ...
        socket.on(exports.socketEvents.CALL_OFFER, async ({ callId, sessionId, sdp, }) => {
            try {
                if (!user?.userId || !callId || !sessionId || !sdp)
                    return;
                const call = await call_model_1.default.findById(callId);
                if (!call) {
                    socket.emit(exports.socketEvents.CALL_ERROR, {
                ...
                io.to(exports.socketRooms.user(targetUserId)).emit(exports.socketEvents.CALL_OFFER, {
                    callId,
                    sessionId,
        ...
        socket.on(exports.socketEvents.CALL_ANSWER, async ({ callId, sessionId, sdp, }) => {
            try {
                if (!user?.userId || !callId || !sessionId || !sdp)
                    return;
                const call = await call_model_1.default.findById(callId);
                ...
                const targetUserId = isCaller
                    ? call.receiverId.toString()
                    : call.callerId.toString();
                io.to(exports.socketRooms.user(targetUserId)).emit(exports.socketEvents.CALL_ANSWER, {
                    callId,
                    sessionId,
                    fromUserId: user.userId,
                    sdp,
        ...
        socket.on(exports.socketEvents.CALL_ICE_CANDIDATE, async ({ callId, sessionId, candidate, }) => {
            try {
                if (!user?.userId || !callId || !sessionId || !candidate)
                    return;
                const call = await call_model_1.default.findById(callId);
                if (!call) {
                    socket.emit(exports.socketEvents.CALL_ERROR, {
                ...
                const targetUserId = isCaller
                    ? call.receiverId.toString()
                    : call.callerId.toString();
                io.to(exports.socketRooms.user(targetUserId)).emit(exports.socketEvents.CALL_ICE_CANDIDATE, {
                    callId,
                    sessionId,
                    fromUserId: user.userId,
                    candidate,

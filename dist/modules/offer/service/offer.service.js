    ...
    static async acceptOffer(input) {
        ...
        try {
            await dbSession.withTransaction(async () => {
                ...
                sessionDoc = await session_model_1.default
                    .create([
                    {
                        orderId: order._id,
                        offerId: offer._id,
    ...
    static async deleteOffer(input) {
        ...
        try {
            await dbSession.withTransaction(async () => {
                ...
                // accepted offer flow
                relatedSession = await session_model_1.default
                    .findOne({
                    offerId: existingOffer._id,
                })
                    .session(dbSession || null);
                if (relatedSession &&
                    ![session_constants_1.SESSION_STATUS.COMPLETED, session_constants_1.SESSION_STATUS.CANCELLED].includes(relatedSession.status)) {
                ...
                relatedOrder.status = order_constants_1.ORDER_STATUS.PENDING;
        ...
        const orderPayload = await (0, buildSupplierOrderPayload_1.buildSupplierOrderPayload)(relatedOrder._id.toString());
        if (relatedSession) {
            ...
            io.to(socket_1.socketRooms.user(relatedOrder.customerId.toString())).emit(socket_1.socketEvents.SESSION_CANCELLED, {
            ...
            io.to(socket_1.socketRooms.user(supplierId)).emit(socket_1.socketEvents.SESSION_CANCELLED, {
                sessionId: relatedSession._id.toString(),
                session: sessionPayload,
                cancelledBy: "supplier",
                reason: "supplier_deleted_accepted_offer",
            ...
            io.to(socket_1.socketRooms.chat(relatedSession._id.toString())).emit(socket_1.socketEvents.SESSION_CANCELLED, {
                sessionId: relatedSession._id.toString(),
                session: sessionPayload,
                cancelledBy: "supplier",
                reason: "supplier_deleted_accepted_offer",
